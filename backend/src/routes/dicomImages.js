  ...
  try {
    ...
    for (const seriesId of studyData.Series || []) {
      ...
      series.push({
        ...
        seriesDescription:
          seriesInfo.SeriesDescription || `Series ${series.length + 1}`,
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch study metadata: " + error.message });
  }
});
...
console.log("ORTHANC_AUTH:", ORTHANC_AUTH,process.env.ORTHANC_USERNAME ,process.env.ORTHANC_PASSWORD);
    ...
    for (const instanceId of instances) {
      ...
      instanceDetails.push({
        ...
        instanceNumber: parseInt(
          tags.InstanceNumber || instanceDetails.length + 1
        ),
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch instances: " + error.message });
  }
});
...
router.get("/image/:instanceId", async (req, res) => {
  try {
    ...
    const response = await fetch(
      `${ORTHANC_URL}/instances/${instanceId}/${type}`,
      {
        headers: { Authorization: ORTHANC_AUTH },
      }
    );
